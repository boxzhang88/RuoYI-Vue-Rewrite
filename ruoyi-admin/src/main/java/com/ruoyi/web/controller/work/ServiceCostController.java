...
public class ServiceCostController extends BaseController {
    ...
    public AjaxResult importData(MultipartFile file, Long deptId, Long taskId) throws Exception {
        ...
        for (ServiceCost serviceCost : projectWorkers) {
            serviceCost.setDepartmentId(deptId);
            serviceCost.setTaskId(taskId);
            serviceCost.setTaskId(taskId);
            serviceCost.setTaskName(task.getTaskName());
            serviceCost.setDepartmentName(department.getDeptName());
        }
        serviceCostService.saveBatch(projectWorkers);
        return success(projectWorkers.size());
    }

    @RequestMapping("/delServiceProject")
    public AjaxResult delServiceProject(@RequestBody ServiceCost serviceCost) {
        return AjaxResult.success(serviceCostService.remove(new LambdaQueryWrapper<>()));
    }
}
